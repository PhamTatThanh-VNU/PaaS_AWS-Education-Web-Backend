...
const authenticateJWT = async (req, res, next) => {
  ...
  try {
    ...
    if (!key) {
      return res.status(401).json({ success: false, message: 'Invalid token: key not found' });
    }
    ...
    if (decodedToken.client_id !== APP_CLIENT_ID && decodedToken.aud !== APP_CLIENT_ID) {
    ...
    req.user = {
      ...
      username: decodedToken.preferred_username || decodedToken['cognito:username'],
